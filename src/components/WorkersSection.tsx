const WorkersSection = () => {
  return (
    <section className="py-24 md:py-36 bg-foreground text-background">
      <div className="mx-auto px-6 md:px-10 lg:px-16 max-w-[1440px]">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <p className="label-default text-background/60 uppercase tracking-widest mb-4">
            Resident voices
          </p>
          <h2 className="section-title text-background mb-4">What past residents are saying</h2>
          <p className="body-default text-background/70">
            Share testimonials from founders, engineers, and researchers who built in the residency.
          </p>
        </div>
        <div className="grid md:grid-cols-2 gap-6">
          {Array.from({ length: 2 }).map((_, index) => (
            <div
              key={`chat-${index}`}
              className="rounded-2xl border border-background/15 bg-background/5 p-6 h-40"
            />
          ))}
        </div>
      </div>
    </section>
  );
};

export default WorkersSection;
